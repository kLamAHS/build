...
const schematic=wholeBuilding(plan,model.grid);await writeFile(resolve(folder,'crownward-citadel.glb'),buildingGlb(meshes,plan.name));await writeFile(resolve(folder,'cells.bin'),new Uint8Array(schematic.cells.buffer));await writeFile(resolve(folder,'palette.json'),JSON.stringify(schematic.palette));await writeFile(resolve(folder,'crownward-citadel.litematic'),await litematicaFile(schematic,1700000000000));
