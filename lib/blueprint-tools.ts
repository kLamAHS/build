...
export function validateSettings(input:unknown,base:Settings):Settings {
  ...
  for(const key of ['courtyard','chapel','garden','cellar','essentials'] as const)if(record[key]!==undefined&&typeof record[key]!=='boolean')throw new Error(`${key} must be true or false.`);
...
export function summarizePlan(plan:Plan){return {schemaVersion:plan.schemaVersion,generatorVersion:plan.generatorVersion,name:plan.name,settings:plan.settings,family:plan.family,width:plan.width,depth:plan.depth,minY:plan.minY,maxY:plan.maxY,validation:plan.validation,navigation:{score:plan.navigation.score,maxDepth:plan.navigation.maxDepth,loops:plan.navigation.loops,forcedCrossings:plan.navigation.transits.map(t=>({room:t.name,strands:t.strands.length}))},components:plan.components.map(c=>({id:c.id,name:c.name,kind:c.kind,storeys:c.storeys,topY:c.topY,parentId:c.parentId})),floors:plan.floors.map(f=>({index:f.index,name:f.name,elevation:f.elevation,voids:f.voids.map(v=>v.name),rooms:f.rooms.map(r=>({id:r.id,name:r.name,width:r.bounds.w-1,depth:r.bounds.d-1,floorY:r.floorY,ceilingY:r.ceilingY,area:r.area}))}))};}
