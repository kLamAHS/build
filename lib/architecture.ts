...
export function generateCandidate(settings:Settings,attempt=0):Plan {
  ...
  if(!small&&s.chapel&&s.kind!=='house'){
    for(const c of [service,domestic,hall].filter(Boolean) as BuildingComponent[]){if(attach(c,'chapel','Chapel','n',16,24,1,0))break;}
  }
  ...
  for(const c of components){
    ...
    if(c.kind==='chapel') {room(c,'Chapel','sacred',b,0,6);continue;}
    ...
    for(let f=cellar?-1:0;f<c.storeys;f++){
      ...
      if(b.d<28||b.w<18){
        const band=5,z=rb.z+Math.floor(rb.d/2)-2;
      ...
      const cx=rb.x+Math.floor(rb.w*(f%2===0?.56:.43))+ri(-1,1),cz=Math.min(rb.z+Math.floor(rb.d/2)+ri(-2,1),b.z+b.d-17),band=4;
      ...
      const corners=[{x:rb.x,z:rb.z,w:cx-rb.x,d:cz-rb.z},{x:cx,z:rb.z,w:rb.x+rb.w-cx,d:cz-rb.z},{x:rb.x,z:cz+band,w:sx-rb.x,d:rb.z+rb.d-cz-band},{x:sx,z:cz+band,w:rb.x+rb.w-sx,d:rb.z+rb.d-cz-band}];
      let program:[string,RoomKind][];
      if(f<0)program=[['Wine cellar','storage'],['Root store','storage'],['Strong room','storage']];
      else if(c.kind==='service'&&f===0)program=[['Kitchen','service'],['Pantry & buttery','storage'],['Scullery','service']];
      else if(c.kind==='workshop'&&f===0)program=[['Workshop','service'],['Counting room','study'],['Goods store','storage']];
      else if(c.kind==='tower'&&f===0)program=[['Guardroom','service'],['Armoury','storage'],['Steward’s office','study']];
      else if(f===0&&c.kind==='domestic')program=[['Solar','study'],['Withdrawing room','study'],['Household dining','service']];
      else if(f===c.storeys-1&&f>1)program=[['Bedchamber','bedroom'],['Wardrobe','storage'],['Private study','study']];
      else program=[['Bedchamber','bedroom'],['Guest chamber','bedroom'],['Linen room','storage']];
      if(c.kind==='tower'&&f>0){
        room(c,f===c.storeys-1?'Tower chamber':'Solar chamber','bedroom',{x:rb.x,z:rb.z,w:rb.w,d:cz-rb.z},y);
        room(c,'Antechamber','study',corners[2],y);
      }else if(f===c.storeys-1&&f>1){
        room(c,'Gabled bedchamber','bedroom',{x:rb.x,z:rb.z,w:rb.w,d:cz-rb.z},y,y+4);
        room(c,'Wardrobe & study','study',corners[2],y,y+4);
      }else corners.slice(0,3).forEach((r,i)=>room(c,program[i][0],program[i][1],r,y));
      if(hasStairs){const r=room(c,'Stair hall','stairs',corners[3],y);stairRooms.push(r);}
      else room(c,c.kind==='service'?'Bread oven':'Household store',c.kind==='service'?'service':'storage',corners[3],y);
    }
  ...
  const candidates:{a:Room;b:Room;axis:'x'|'z';fixed:number;pos:number;priority:number}[]=[];
  ...
  for(let i=0;i<p.rooms.length;i++)for(let j=i+1;j<p.rooms.length;j++){
    ...
    const publicRoom=(r:Room)=>['circulation','stairs','gallery','hall'].includes(r.kind);
    candidates.push({a,b,axis,fixed,pos,priority:(publicRoom(a)&&publicRoom(b)?0:publicRoom(a)||publicRoom(b)?1:5)+(a.componentId===b.componentId?0:2)});
  }
  ...
  for(const st of p.stairs){p.connections.push(st.roomIds as [string,string]);join(st.roomIds[0],st.roomIds[1]);}
  for(const edge of candidates.sort((a,b)=>a.priority-b.priority)){
    if(find(edge.a.id)===find(edge.b.id)&&edge.priority>0)continue;
    const pos=edge.pos;
    const o:Opening={id:`o${p.openings.length}`,type:'door',axis:edge.axis,x:edge.axis==='x'?edge.fixed:pos,y:edge.a.floorY+1,z:edge.axis==='z'?edge.fixed:pos,width:2,height:3,roomIds:[edge.a.id,edge.b.id]};
    p.openings.push(o);p.connections.push([edge.a.id,edge.b.id]);join(edge.a.id,edge.b.id);
  }
  for(const r of p.rooms){
    const degree=p.connections.filter(pair=>pair.includes(r.id)).length;
    if(r.kind==='bedroom'&&degree>1){r.kind='circulation';r.name='Private antechamber';}
  }
...
export function validatePlan(p:Plan):Plan['validation']{
  ...
  for(const r of p.rooms){if(r.bounds.w<4||r.bounds.d<4)issues.push(`${r.name} is too narrow.`);if(r.kind==='bedroom'&&p.connections.filter(a=>a.includes(r.id)).length>1)issues.push('A public route passes through a bedroom.');}
...
export function tryGenerate(settings:Settings):GenerationResult {
  ...
  for(let attempt=0;attempt<5;attempt++){
    try{const plan=generateCandidate(settings,attempt);if(plan.validation.valid){const issues=auditArchitecture(plan);if(!issues.length)return {ok:true,plan};reason=issues.join(' ');}else reason=plan.validation.issues.join(' ');}catch(error){return {ok:false,error:error instanceof Error?error.message:'Invalid settings.'};}
  }
