...
import { wholeBuilding, floorOutline, cellIndex, BLOCKS } from './litematica.ts';
...
for (const kind of ['castle','manor','house'] as const) for (const family of FAMILIES[kind]) {
  void test(`architectural export preserves the generated ${family.id} plan`,()=>{
    ...
    raw.forEach((x,y,z,value)=>{
      if(raw.kindAt(x,y,z)!=='roof')assert.equal(model.grid.get(x,y,z),value,`Structural block changed at ${x},${y},${z}`);
    });
    ...
    model.grid.forEach((x,y,z,value)=>{
      ...
      assert.equal(schematic.palette[schematic.cells[at]],model.grid.stateAt(x,y,z)?.key??BLOCKS[value&15]??'minecraft:stone');
    });
