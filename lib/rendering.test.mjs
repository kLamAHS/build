import assert from 'node:assert/strict';
import { test } from 'node:test';
import { readFile } from 'node:fs/promises';
import { createRequire } from 'node:module';
import { pathToFileURL } from 'node:url';
import ts from 'typescript';
import React from 'react';
import { renderToStaticMarkup } from 'react-dom/server';
import { generatePlan, DEFAULT_SETTINGS } from './generator.ts';

test('SVG stays identical when engines differ at the final trigonometric decimal',async()=>{
 const require=createRequire(import.meta.url);
 let source=await readFile(new URL('../components/plan-drawing.tsx',import.meta.url),'utf8');
 source=source.replace(/'@\/lib\/([\w-]+)'/g,(_,name)=>JSON.stringify(new URL(`./${name}.ts`,import.meta.url).href));
 let compiled=ts.transpileModule(source,{compilerOptions:{jsx:ts.JsxEmit.ReactJSX,module:ts.ModuleKind.ESNext,target:ts.ScriptTarget.ES2022}}).outputText;
 for(const name of ['react/jsx-runtime','react'])compiled=compiled.replaceAll(`from "${name}"`,`from ${JSON.stringify(pathToFileURL(require.resolve(name)).href)}`).replaceAll(`from '${name}'`,`from ${JSON.stringify(pathToFileURL(require.resolve(name)).href)}`);
 const {PlanDrawing}=await import(`data:text/javascript;base64,${Buffer.from(compiled).toString('base64')}`);
 const plan=generatePlan(DEFAULT_SETTINGS);
 const render=(diagnostics='off')=>renderToStaticMarkup(React.createElement('svg',null,React.createElement(PlanDrawing,{plan,floor:plan.floors[0],options:{labels:true,furniture:true,grid:true,dimensions:true,colors:true,diagnostics}})));
 // Every stage can show its working, and each overlay draws something rather than quietly rendering nothing.
 for(const mode of ['volumes','circulation','facade','fit']){
  const svg=render(mode);
  assert.ok(svg.length>render('off').length/3,`the ${mode} overlay drew almost nothing`);
  assert.notEqual(svg,render('off'),`the ${mode} overlay is indistinguishable from the finished drawing`);
 }
 // The volumes overlay names the build each range belongs to, so a seam can be traced to the stage that made it.
 const volumes=render('volumes');
 assert.ok(/retained core|single build|build \d/.test(volumes),'the volumes overlay does not say which build a range belongs to');
 const baseline=render();
 const sin=Math.sin,cos=Math.cos;
 try{Math.sin=x=>sin(x)*(1+Number.EPSILON);Math.cos=x=>cos(x)*(1+Number.EPSILON);assert.equal(render(),baseline);}finally{Math.sin=sin;Math.cos=cos;}
 assert.ok(!baseline.includes('NaN'));
});
